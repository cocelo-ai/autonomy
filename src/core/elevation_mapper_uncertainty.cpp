...
Matrix6 poseCovariance(const Pose2_5D &pose,
  ...
  const double orientation_variance =
      std::pow(config.pose_min_orientation_std_deg * 0.01745329251994329577, 2);
  for (int index = 0; index < 3; ++index) {
    covariance(index, index) =
        std::max(covariance(index, index), position_variance);
    covariance(index + 3, index + 3) =
        std::max(covariance(index + 3, index + 3), orientation_variance);
  }
...
void ElevationMapper::addRollingSamples(SampleMap &samples,
  ...
  const Eigen::Vector3d lever(point.x - robot.x, point.y - robot.y,
                              point.z - robot.z);
  ...
  const double orientation_limit =
      config_.pose_max_orientation_std_deg * 0.01745329251994329577;
  ...
  const Eigen::Vector3d sensor_lever =
      std::isfinite(point.origin_x) && std::isfinite(point.origin_y) &&
              std::isfinite(point.origin_z)
          ? Eigen::Vector3d(point.x - point.origin_x, point.y - point.origin_y,
                            point.z - point.origin_z)
          : lever;
  const double sensor_variance =
      (is_d435 ? config_.d435_base_variance : config_.rolling_base_variance) +
      (is_d435 ? config_.d435_range_variance_factor
               : config_.rolling_range_variance_factor) *
          sensor_lever.squaredNorm();
  const double translation_std =
      is_d435 ? config_.d435_extrinsic_translation_std_m
              : config_.lidar_extrinsic_translation_std_m;
  const double rotation_std =
      (is_d435 ? config_.d435_extrinsic_rotation_std_deg
               : config_.lidar_extrinsic_rotation_std_deg) *
      0.01745329251994329577;
  const double calibration_variance =
      std::pow(translation_std, 2) +
      std::pow(rotation_std, 2) * (sensor_lever.x() * sensor_lever.x() +
                                   sensor_lever.y() * sensor_lever.y());

  Eigen::Matrix<double, 1, 6> height_jacobian =
  ...
  Eigen::Matrix2d xy_covariance =
      xy_jacobian * covariance * xy_jacobian.transpose();
  ...
  const int radius =
      std::min(config_.pose_splat_max_cells,
               xy_std <= 0.25 * config_.grid.resolution
                   ? 0
                   : static_cast<int>(
                         std::ceil(2.0 * xy_std / config_.grid.resolution)));
  const Eigen::Matrix2d inverse = xy_covariance.inverse();
  std::vector<std::pair<CellKey, double>> weights;
  for (int y = -radius; y <= radius; ++y) {
    for (int x = -radius; x <= radius; ++x) {
      const Eigen::Vector2d offset(
          (static_cast<double>(center.x + x) + 0.5) * config_.grid.resolution -
              point.x,
          (static_cast<double>(center.y + y) + 0.5) * config_.grid.resolution -
              point.y);
      const double mahalanobis = offset.transpose() * inverse * offset;
      if (mahalanobis <= 4.0) {
        weights.push_back(
            {{center.x + x, center.y + y}, std::exp(-0.5 * mahalanobis)});
      }
    }
  }
  if (weights.empty()) {
  ...
  for (const auto &[key, weight] : weights) {
    ...
    samples[key].push_back(
        {point.z,
         static_cast<float>(height_variance / std::max(0.01, probability))});
  }
}
