...
void LoadParamFromRos(rclcpp::Node& node)
{
  ...
  node.declare_parameter<double>("lio.slam.accumulation_interval_sec", 0.1);
  node.get_parameter("lio.slam.accumulation_interval_sec",
                     g_slam_accumulation_interval_sec);
  ...
  if (g_slam_keyframe_distance <= 0.0F || g_slam_keyframe_yaw_deg <= 0.0F ||
      g_slam_keyframe_leaf_size <= 0.0F || g_slam_accumulation_interval_sec < 0.0) {
    throw std::invalid_argument("lio.slam keyframe/accumulation parameters are invalid");
  }

  node.declare_parameter<bool>("lio.elevation.enable", true);
  node.get_parameter("lio.elevation.enable", g_elevation_enable);
  load_float("lio.elevation.radius", 3.0, g_elevation_radius);
  load_float("lio.elevation.leaf_size", 0.10, g_elevation_leaf_size);
  node.declare_parameter<double>("lio.elevation.publish_rate_hz", 5.0);
  node.get_parameter("lio.elevation.publish_rate_hz", g_elevation_publish_rate_hz);
  if (g_elevation_radius <= 0.0F || g_elevation_leaf_size <= 0.0F ||
      g_elevation_publish_rate_hz <= 0.0) {
    throw std::invalid_argument("lio.elevation parameters must be positive");
  }
