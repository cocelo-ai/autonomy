...
void ROSWrapper::pub_elevation_cloud(const CloudPtr& pc, double time) {
  if (!pub_elevation_cloud_ || !pc || pc->empty()) {
    return;
  }
  sensor_msgs::msg::PointCloud2 cloud;
  pcl::toROSMsg(*pc, cloud);
  cloud.header.frame_id = g_global_frame;
  cloud.header.stamp = toRosTime(time);
  pub_elevation_cloud_->publish(cloud);
}

void ROSWrapper::pub_elevation_crop(const BASIC::V3& center, const double radius,
                                    const double time) {
  if (!global_map_ || global_map_->empty() || radius <= 0.0) {
    return;
  }
  const float radius_squared = static_cast<float>(radius * radius);
  BASIC::CloudPtr crop(new BASIC::PointCloudType());
  crop->reserve(global_map_->size() / 10U + 1U);
  for (const auto& point : global_map_->points) {
    const float dx = point.x - center.x();
    const float dy = point.y - center.y();
    if (dx * dx + dy * dy <= radius_squared) {
      crop->push_back(point);
    }
  }
  pub_elevation_cloud(crop, time);
}
