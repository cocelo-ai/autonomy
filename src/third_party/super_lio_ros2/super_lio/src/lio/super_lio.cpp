...
void SuperLIO::init(){
  ...
  if (g_slam_enable) {
    ...
    pose_graph_ = std::make_unique<PoseGraph>(PoseGraph::Options{
        ...
        g_slam_keyframe_leaf_size, g_slam_accumulation_interval_sec,
        g_slam_loop_descriptor_threshold,
...
void SuperLIO::Output(){
  ...
  if (g_elevation_enable &&
      (last_elevation_publish_time_ < 0.0 ||
       state.timestamp - last_elevation_publish_time_ >= 1.0 / g_elevation_publish_rate_hz)) {
    last_elevation_publish_time_ = state.timestamp;
    if (pose_graph_) {
      data_wrapper_->pub_elevation_cloud(
          pose_graph_->buildLocalMap(global_pose.t_, g_elevation_radius, g_elevation_leaf_size),
          state.timestamp);
    } else {
      data_wrapper_->pub_elevation_crop(global_pose.t_, g_elevation_radius, state.timestamp);
    }
  }
