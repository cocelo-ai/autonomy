...
public:
  struct Options {
    ...
    double accumulation_interval_sec{0.10};
  ...
  BASIC::CloudPtr buildLocalMap(const BASIC::V3& center, float radius,
...
private:
  ...
  void accumulateLatest(const BASIC::SE3& odom_pose,
                        const BASIC::CloudPtr& cloud, double stamp);
  ...
  double last_accumulation_stamp_{-1.0};
  std::size_t accumulated_frame_count_{0U};
