...
bool ElevationMapping::readParameters() {
  ...
  map_.configureUnderlyingMap(map_.underlyingMapTopic_);
...
void ElevationMapping::pointCloudCallback(sensor_msgs::msg::PointCloud2::ConstSharedPtr pointCloudMsg, bool publishPointCloud,
  ...
  // RealSense and Super-LIO PointCloud2 messages normally do not carry the
  // optional confidence_ratio field.  The upstream point type then contains
  // zero/undefined confidence, which would make structured-light variance
  // infinite and discard every D435 observation.  Missing confidence means a
  // neutral weight of one; genuine positive confidence values are preserved.
  for (auto& point : pointCloud->points) {
    if (!std::isfinite(point.confidence_ratio) || point.confidence_ratio <= 0.0F) {
